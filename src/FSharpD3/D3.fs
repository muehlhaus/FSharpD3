...
module D3 =
    ...
    module Selection =
        ...
        //let on f (selection: D3.Selection) = selection.on. call(System.Func<_, _>(f))
    ...
    [<AutoOpen>]
    module Layout =
        let force () = Globals.d3.layout.force()
        let drag  (forceLayout: D3.Layout.ForceLayout) = forceLayout.drag() |> ignore //.Invoke() //
        let nodes array (forceLayout: D3.Layout.ForceLayout) = forceLayout.nodes.Invoke(array |> Array.map unbox)
        let links array (forceLayout: D3.Layout.ForceLayout) = forceLayout.links.Invoke(array |> Array.map unbox)
        let size width height (forceLayout: D3.Layout.ForceLayout) = forceLayout.size.Invoke([|width;height|])
        let linkDistance (linkDistance:float) (forceLayout: D3.Layout.ForceLayout) = forceLayout.linkDistance.Invoke(linkDistance)
        let linkDistanceF f (forceLayout: D3.Layout.ForceLayout) = forceLayout.linkDistance.Invoke(System.Func<obj,float,float>(f))
        let linkStrength (f:obj -> float -> float) (forceLayout: D3.Layout.ForceLayout) = forceLayout.linkStrength.Invoke(System.Func<_,float,float>(f))
        
        let charge (c:float) (forceLayout: D3.Layout.ForceLayout) = forceLayout.charge.Invoke(c)
        let start (forceLayout: D3.Layout.ForceLayout) =  forceLayout.start()
        let tick (forceLayout: D3.Layout.ForceLayout) =  forceLayout.tick()
        let on (name:string) f (forceLayout: D3.Layout.ForceLayout) = forceLayout.on(name,(System.Func<unit>(f)))
...
//    type D3.Layout.ForceLayout with
//        member self.Compute() =
//            downcast self.Invoke()  
